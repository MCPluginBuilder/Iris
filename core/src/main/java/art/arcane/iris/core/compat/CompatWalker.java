 ...
 * <p>
 * Boundaries: {@link IrisObjectPlacement} is never descended (placements evaluate their own pools), match-side block
 * lists are ignored (a missing block that is only matched is harmless), registrant references
 * ({@code @RegistryListResource}) belong to the pool cascade, and identity cycles are cut.
 */
final class CompatWalker {
    ...
    private boolean walkValue(Object value, String path) {
        if (value == null || value instanceof IrisObjectPlacement) {
            return false;
        }
        if (value instanceof IrisBlockData block) {
    ...
    static boolean isIrisObject(Class<?> type) {
        return type.getName().startsWith(IRIS_PACKAGE)
                ...
                && !type.isRecord()
                && !type.isAnnotation()
                && !IrisObjectPlacement.class.isAssignableFrom(type);
    }
