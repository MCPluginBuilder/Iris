...
public final class VisionGUI extends JPanel implements MouseWheelListener, KeyListener, MouseMotionListener, MouseInputListener {
    ...
    private JToggleButton qualityToggle;
    ...
    private static JPanel buildToolbar(VisionGUI vision) {
        ...
        vision.qualityToggle = createToggle(
                IrisLanguage.plain(DesktopUiMessages.VISION_LOW_QUALITY_SHORT),
                vision.lowQuality
        );
        vision.qualityToggle.addActionListener(event -> vision.toggleQuality());
        trailing.add(vision.qualityToggle);
    ...
    private void renderTiles(Graphics2D canvas) {
        ...
        double translationX = frame.spec().zoom() == zoom
                ? (frame.spec().centerX() - centerX) / currentBlocksPerPixel
                : 0D;
        double translationZ = frame.spec().zoom() == zoom
                ? (frame.spec().centerZ() - centerZ) / currentBlocksPerPixel
                : 0D;
        int tilePixels = VisionRenderController.TILE_PIXELS;
        for (VisionRenderController.VisibleTile tile : frame.tiles()) {
            int screenX = (int) Math.round(tile.screenX() + translationX);
            int screenY = (int) Math.round(tile.screenY() + translationZ);
            canvas.setColor(((tile.tileX() ^ tile.tileZ()) & 1L) == 0L ? TILE_EMPTY_A : TILE_EMPTY_B);
            canvas.fillRect(screenX, screenY, tilePixels, tilePixels);
            BufferedImage image = controller.image(frame, tile);
            if (image != null) {
                canvas.drawImage(image, screenX, screenY, tilePixels, tilePixels, null);
            }
        }
    }
    ...
    private void renderStatus(Graphics2D canvas) {
        ...
        String middle = progress.refinedReady() + "/" + progress.total() + " HD  "
                + progress.previewReady() + "/" + progress.total() + " preview  "
    ...
    private void renderHelp(Graphics2D canvas) {
        List<String> keys = List.of("/", "R", "F", "+/-", "\\", "M", "P", "G", "Shift", "Ctrl+Click", "Alt+Click");
        List<String> descriptions = List.of(
                ...
                IrisLanguage.plain(DesktopUiMessages.VISION_HELP_QUALITY),
    ...
    private void renderDebug(Graphics2D canvas) {
        ...
        lines.add(IrisLanguage.plain(
                ...
                MessageArgument.trusted("high", progress.refinedReady()),
                MessageArgument.trusted("low", progress.previewReady())
        ));
        lines.add(IrisLanguage.plain(
                DesktopUiMessages.VISION_WORKERS,
                MessageArgument.trusted("high", progress.active()),
                MessageArgument.trusted("low", progress.queued())
        ));
        lines.add(IrisLanguage.plain(
    ...
    private static HoverInfo sampleHover(Engine engine, double worldX, double worldZ) {
        IrisBiome biome = engine.getComplex().getTrueBiomeStream().get(worldX, worldZ);
    ...
    private void toggleQuality() {
        ...
        requestRender();
        notifyUser(IrisLanguage.plain(
                lowQuality ? DesktopUiMessages.VISION_LOW_QUALITY : DesktopUiMessages.VISION_HIGH_QUALITY
        ));
    }
    ...
    private void syncControls() {
        ...
        try {
            ...
            if (qualityToggle != null) {
                qualityToggle.setSelected(lowQuality);
            }
        } finally {
    ...
    private void changeZoom(int requestedZoom, Point anchor) {
        int boundedZoom = Math.max(
                VisionRenderController.MINIMUM_ZOOM,
                Math.min(VisionRenderController.MAXIMUM_ZOOM, requestedZoom)
        );
        if (boundedZoom == zoom) {
            return;
        }
        Point effectiveAnchor = anchor == null ? new Point(getWidth() / 2, getHeight() / 2) : anchor;
        double anchorWorldX = screenToWorldX(effectiveAnchor.x);
        double anchorWorldZ = screenToWorldZ(effectiveAnchor.y);
        zoom = boundedZoom;
        centerX = anchorWorldX - (effectiveAnchor.x - getWidth() * 0.5D) * blocksPerPixel();
        centerZ = anchorWorldZ - (effectiveAnchor.y - getHeight() * 0.5D) * blocksPerPixel();
        requestRender();
    }
    ...
    public void mouseWheelMoved(MouseWheelEvent event) {
        if (!event.isControlDown()) {
            changeZoom(zoom + event.getWheelRotation(), event.getPoint());
        }
    }
