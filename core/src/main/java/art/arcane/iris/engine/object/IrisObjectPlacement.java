...
public class IrisObjectPlacement {
    ...
    private CompatStatus computeCompat(IrisData data) {
        ...
        CompatFinding blocked = evaluatePlacementBlocks(data, gate, subject, reasons);
        record(report, reasons);
        ...
        for (int i = 0; i < place.size(); i++) {
            ...
            KList<String> missing = unplaceableKeys(objectKey, data, gate, subject + " place[" + i + "]", objectReasons);
            record(report, objectReasons);
     ...
     *
     * @return the finding that excludes this placement, or null when every declared block resolves
     */
    private CompatFinding evaluatePlacementBlocks(IrisData data, ContentGate gate, String subject, KList<CompatFinding> reasons) {
        CompatFinding blocked = null;

        for (int i = 0; i < edit.size(); i++) {
            IrisObjectReplace rule = edit.get(i);

            if (rule == null || rule.getReplace() == null) {
                continue;
            }
            ...
            for (int j = 0; j < palette.size(); j++) {
                CompatFinding finding = evaluateBlockEntry(palette.get(j), data, gate, subject,
                        "edit[" + i + "].replace.palette[" + j + "]", reasons);
    ...
    /** Records what happened to one declared block entry; returns the finding when it excludes the placement. */
    private CompatFinding evaluateBlockEntry(IrisBlockData block, IrisData data, ContentGate gate, String subject,
                                             String detail, KList<CompatFinding> reasons) {
        if (block == null) {
            return null;
        }
        ...
        if (resolution.substituted()) {
            reasons.add(new CompatFinding(CompatRegistry.BLOCK, resolution.key(), CompatAction.SUBSTITUTED,
                    "placement", subject, detail + " (using " + resolution.replacement() + ")"));
        }
     ...
     * {@link #evaluatePlacementBlocks}. A rule below chance 1 does not cover the key: the original block can still be
     * rolled.
     */
    private KList<String> unplaceableKeys(String objectKey, IrisData data, ContentGate gate, String detail, KList<CompatFinding> reasons) {
            ...
            if (resolution != null) {
                if (resolution.substituted()) {
                    reasons.add(new CompatFinding(CompatRegistry.BLOCK, base, CompatAction.SUBSTITUTED,
                            "object", objectKey, detail + " (using " + resolution.resolvedKey() + ")"));
                }

                continue;
            }

            if (coveredByEdit(paletteKey)) {
                continue;
            }
    ...
    private boolean coveredByEdit(String paletteKey) {
     ...
     */
    private static BlockCompat resolveBlockEntry(IrisBlockData block, IrisData data, ContentGate gate, KList<IrisBlockData> seen) {
    ...
    private static void record(PackCompatReport report, KList<CompatFinding> findings) {
    ...
    private record BlockCompat(String key, String replacement, boolean missing, boolean substituted) {
    }
